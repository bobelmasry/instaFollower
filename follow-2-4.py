...
def clickFollow(x, y):
    ...
    pyautogui.click()
    time_sleepRand = round(random.uniform(3, 5), 2)
    time.sleep(time_sleepRand)
    screenshot = ImageGrab.grab()
    screenshot.save('insta-image2.png', 'PNG')
...
while True:
    ...
    while followCount < random_follow:
        if hasFollow('insta-image2.png')[0]:
            clickFollow(hasFollow('insta-image2.png')[1][0], hasFollow('insta-image2.png')[1][1]) # x and y coordinates
            followCount += 1
            screenshot = ImageGrab.grab()
            screenshot.save('insta-image2.png', 'PNG')
        else:
